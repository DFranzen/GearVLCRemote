...
var app = {
	...
	back: function (ev) {
		...
		app.show(app.viewStack[app.viewStack.length - 1]);
    ...
    addVLCconnection: function() {
    	...
    	app.showSpinner("Waiting for " + server.serverName + " on " + server.serverIP);
    	vlc.connect(vlc.serverList.length - 1,true);
    },
    initVLCconnection: function(id) {
    	return function () {
    		var index=id;
    		app.showSpinner("Waiting for " + vlc.serverList[index].serverName + " on " + vlc.serverList[index].serverIP);
    		vlc.connect(index);
    	};
    },
    /*value: 0-200*/
